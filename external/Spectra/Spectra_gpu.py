...
class SPECTRA_Model:
    ...
    def train(
        ...
        for i in (bar:=tqdm(range(num_epochs))):
            ...
            if tot < min_loss:
                ...
                if early_stop_counter >= early_stop_patience:
                    print(
                        "EARLY STOPPING"
                        f"Last {early_stop_patience} epochs had no improvement"
                        f"After {i} epochs, the loss is {tot}"
                    )
                    break
            bar.set_description(f'Loss: {tot:.2f}')
